...
my $obs = countValidatedPasswords(\@testdata);
is($obs, $expected, "test data");

my @data = grep {/./} <DATA>;
my $obs2=countValidatedPasswords(\@data);
is($obs2, 636, "real data");

sub countValidatedPasswords{
  my($data) = @_;
  ...
  for my $rules(@$data){
    ...
    my($min,$max,$letter,$p);
    if($rules =~ /(\d+)-(\d+) (\w): (\w+)/){
      ($min,$max,$letter,$p) = ($1,$2,$3,$4);
    } else {
      die "Could not parse $rules";
    }

    # Count how many times the target letter appears
    my $letterCounter=0;
    for(my $i=0;$i<length($p);$i++){
      if(substr($p,$i,1) eq $letter){
        $letterCounter++;
      }
    }

    #note "$rules .. $letterCounter";
