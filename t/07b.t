...
subtest 'sample data' => sub{
  plan tests => 2;
  my $exp = 32;
  my $rulesStr="light red bags contain 1 bright white bag, 2 muted yellow bags.
...
";
  my $rule  = parseRules($rulesStr);
  my $count = howManyAreIn("shiny gold", $rule);
  $count--; # don't include main bag
  is($count, $exp, substr($rulesStr, 0, 30).'...');

  my $exp2 = 126;
  my $rulesStr2 = "shiny gold bags contain 2 dark red bags.
...
";
  my $rule2  = parseRules($rulesStr2);
  my $count2 = howManyAreIn("shiny gold", $rule2);
  $count2--; # don't include main bag
  is($count2, $exp2, substr($rulesStr2, 0, 30).'...');
};
  ...
  for my $str(split(/\n/, $rulesStr)){
    ...
    if($str =~ /(.+) bags contain (.+)\.?/){
      ...
      if($containers =~ /no other bags/){
        ...
        while($containers =~ /(\d+) (.+?) bags?/g){
          my $count = $1;
          my $type  = $2;
          $rules{$container}{$type}=$count;
        }
      }

    }
    else {
      BAIL_OUT("ERROR: could not parse for a container and containees: $str");
    }
  }
  #note Dumper \%rules; BAIL_OUT("DEBUG");
...
sub howManyAreIn{
  ...
  for my $subBag(keys(%{ $$rules{$bag} })){
    my $count = $$rules{$bag}{$subBag};
    for(1..$count){
      $numBags += howManyAreIn($subBag, $rules);
    }
  }
