...
use constant FLOOR    => '.';
use constant OCCUPIED => '#';
use constant EMPTY    => 'L';
...
subtest 'sample data' => sub{
  ...
  my $obs = countOccupiedSeats($input);
...
sub countOccupiedSeats{
  my($str) = @_;
  my $seatInfo = parseSeatsString($str);
  
  my $numStates = 0;
  my $currState = $seatInfo;
  my $nextState = nextState($currState);
  while(seatsArrToStr($nextState) ne seatsArrToStr($currState)){
    $numStates++;
    $currState = $nextState;
    $nextState = nextState($nextState);
    #note seatsArrToStr($currState);
    #note " $numStates ";
  }
  note seatsArrToStr($nextState);

  my $numOccupied = () = seatsArrToStr($nextState) =~ /#/g;
  return $numOccupied;
}
...
sub nextState{
  ...
  my $numRows = @$seatArr;
  my $numCols = @{ $$seatArr[0] };
  for(my $i=0;$i<$numRows;$i++){
    for(my $j=0;$j<$numCols;$j++){
      # Spaces that are floor don't get changed between states
      if($$seatArr[$i][$j] eq FLOOR){
        $nextState[$i][$j] = FLOOR;
        next;
      }

      # Count how many seats are occupied around this one:
      #   Zero means that this seat gets occupied
      #   If at least five seats are occupied and this seat
      #   is occupied, then this seat becomes empty.
      #   Otherwise, no change.
      my $numSurroundingOccupied = 0;

      for my $dx(-1,0,+1){
        for my $dy(-1,0,+1){
          next if($dx==0 && $dy==0);
          if(seatInDirection($i,$j,$dx,$dy,$seatArr)){
            $numSurroundingOccupied++;
          }
        }
      }


      #note "$i,$j $numSurroundingOccupied" if($i==0);
      if($numSurroundingOccupied >= 5 && $$seatArr[$i][$j] eq OCCUPIED){
        $nextState[$i][$j] = EMPTY;
      }
      elsif($numSurroundingOccupied == 0){
        $nextState[$i][$j] = OCCUPIED;
      }
      else{
        $nextState[$i][$j] = $$seatArr[$i][$j];
      }

      if(!defined($nextState[$i][$j])){
        BAIL_OUT("ERROR: did not define the state for $i,$j. State was\n"
          . seatsArrToStr($seatArr));
      }
    }
  }
...
sub seatInDirection{
  ...
  while(1){
    ...
    my $seatStatus = $$state[$x][$y];

    if($seatStatus eq FLOOR){
      next;
    }
    elsif($seatStatus eq OCCUPIED){
      return 1;
    }
    elsif($seatStatus eq EMPTY){
      return 0;
    }
    else{
      BAIL_OUT("INTERNAL ERROR: did not understand seat status '$seatStatus'");
    }

    if(++$fieldOfVision > 99999){
      BAIL_OUT("infinite loop on vision");
    }
  }
  return 0;
}
...
sub parseSeatsString{
  my($str) = @_;
  my @arr;

  my @row = grep {/\S+/} split(/\s*\n\s*/,$str);
  for my $r(@row){
    my @col = split(//, $r);
    push(@arr, \@col);
  }
