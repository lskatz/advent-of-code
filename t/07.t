...
";
  ...
  my $count = howManyContain("shiny gold", $rule);
...
sub howManyContain{
  ...
  my $containerCounter = 0;
  for my $container(keys(%$rules)){
    #note "QUERY $needle in $container";
    my $containsIt = doesItContain($container, $needle, $rules);
    #note "    RESULT $containsIt";
    if($containsIt){
      $containerCounter++;
    }
  }
...
# Recursive
sub doesItContain{
  my($thisContainer, $needle, $rules) = @_;

  my $doesItContain = 0;
  while(my($subContainer, $count) = each(%{ $$rules{$thisContainer} })){
    if($subContainer eq $needle){
      $doesItContain = 1;
    } else {
      $doesItContain += doesItContain($subContainer, $needle, $rules);
    }
  }
