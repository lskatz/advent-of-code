...
sub moveShip{
  ...
  for (@instruction){
    ...
    note "DEBUG"; my($y,$x, $dx, $dy, $degrees,);
    ...
    # F means to move ship to the waypoint
    # int times
    elsif($cmd eq 'F'){
      my $xDistFromWaypoint = abs($ship{x} - $waypoint{x});
      my $yDistFromWaypoint = abs($ship{y} - $waypoint{y});
      my $distanceFromWaypoint = sqrt( $xDistFromWaypoint**2 + $yDistFromWaypoint**2 );
      my $radiansToWaypoint = atan2($yDistFromWaypoint, $xDistFromWaypoint);
    }
    else{
