...
my @expected = (610,3454);

# Do a while loop to look at sample data _and_ real data
while(my $path1 = <DATA>){
  my $path2 = <DATA>;
  chomp($path1,$path2);

  my $range1 = findRange($path1);
  my $range2 = findRange($path2);
  my $intersection = findIntersections($range1, $range2);
  my $distFromOrigin=manhattenDistances($intersection);

  my @sortedCoordinates = sort{$$a{steps} <=> $$b{steps}} @$distFromOrigin;

  my $expected = shift(@expected);
  is($sortedCoordinates[0]{steps}, $expected, "Expected $expected");
}

# try 1 was too high: ÿ3461
# try 2 was too high (trying zero-based???): ÿ3460
# try 3 was too low (trying dist instead of steps???): ÿ340
# try 4 just for fun, no feedback given: 3000
# try 5: 3456
# try 6: 3454
...
sub findRange{
  ...
  my $steps = 0;
  for (split(/,/, $path)){
    my $direction = uc substr($_, 0, 1);
    my $length    = substr($_, 1);
    if($direction eq 'R'){
      my($min, $max) = ($x, $x+$length);
      for my $localX ($min..$max){
        $range{$y}{$localX} //= $steps;
        $steps++;
      }
      $x = $max;
    } elsif($direction eq 'U'){
      my($min, $max) = ($y, $y+$length);
      for my $localY ($min..$max){
        $range{$localY}{$x} //= $steps;
        $steps++;
      }
      $y = $max;
    } elsif($direction eq 'L'){
      my($min, $max) = ($x-$length, $x);
      for my $localX ($min..$max){
        $range{$y}{$localX} //= $steps;
        $steps++;
      }
      ...
      die "ERROR: do not understand direction $direction";
    }
    $steps--; # one step back to keep count???
    #print "$x,$y => $steps\n";
  }
...
sub findIntersections{
  ...
  my @intersection;
  while(my($y, $xHash) = each(%$range1)){
    while(my($x, $steps) = each(%$xHash)){
      next if($x==$originX && $y==$originY);
      if($$range2{$y}{$x}){
        my $totalSteps = $steps + $$range2{$y}{$x};
        #print "$totalSteps\t<= $x,$y => $steps + ".$$range2{$y}{$x}." = $totalSteps\n";
        push(@intersection, {x=>$x, y=>$y, steps=>$totalSteps});
      }
    }
  }
  return \@intersection;
...
sub manhattenDistances{
  my ($intersection) = @_;
  my @dist;

  for my $coordinates(@$intersection){
    my $dist = abs($originX - $$coordinates{x}) 
             + abs($originY - $$coordinates{y});
    push(@dist, {
        x    =>$$coordinates{x},
        y    =>$$coordinates{y},
