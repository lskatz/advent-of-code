...
subtest 'sample data' => sub{
  ...
  my $obsTotal=0;
  while(my($answers, $exp) = each(%expected)){
    my $obs = countAnswers($answers);
    is($obs, $exp, $answers);
    $obsTotal+=$obs;
  }

  is($obsTotal, $expTotal, "The total");
};
...
subtest 'real data' => sub{
  ...
  while(<DATA>){
    $currEntry .= $_;
    if(/^\s*$/){
      my $obs = countAnswers($currEntry);
      $obsTotal += $obs;
      $currEntry = "";
    }
  }

  is($obsTotal, 3254, "The total");
};
...
sub countAnswers{
  ...
  my @person = grep{/./} split(/\n/, $answers);

  my %bool;
  for my $person(@person){
    for my $yesAns(split(//, $person)){
      $bool{$yesAns}++;
    }
  }

  my $intersectionCounter=0;
  while(my($bool, $count) = each(%bool)){
    if($count == scalar(@person)){
      $intersectionCounter++;
    }
  }
