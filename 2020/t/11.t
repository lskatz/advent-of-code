...
sub nextState{
  ...
  for(my $i=0;$i<$numRows;$i++){
    for(my $j=0;$j<$numCols;$j++){
      ...
      #   If at least four seats are occupied and this seat
      #   is occupied, then this seat becomes empty.
      #   Otherwise, no change.
      my $numSurroundingOccupied = 0;
      for my $x(-1..1){
        my $adjacentRow = $i + $x;
        next if($adjacentRow < 0);
        next if($adjacentRow >= $numRows);
        for my $y(-1..1){
          my $adjacentCol = $j + $y;
          next if($adjacentCol < 0);
          next if($adjacentCol >= $numCols);
          next if($adjacentRow==$i && $adjacentCol==$j);

          #note "  TEST $adjacentRow,$adjacentCol => ".$$seatArr[$adjacentRow][$adjacentCol] if($i==0);
          if($$seatArr[$adjacentRow][$adjacentCol] eq OCCUPIED){
      ...
      if($numSurroundingOccupied >= 4 && $$seatArr[$i][$j] eq OCCUPIED){
        $nextState[$i][$j] = EMPTY;
      }
