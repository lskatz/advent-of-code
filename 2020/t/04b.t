...
my %fieldsIHaveInvalidated = ();

subtest 'invalid sample set' => sub{
  plan tests=>1;

  my @testdata = split(/\n/, $invalidSampleData);
  my $numValid = countPassports(\@testdata);
  is($numValid, 0);
};

subtest 'valid sample set' => sub{
  plan tests=>1;
...
subtest 'real data' => sub{
  plan tests=>1;
  my @data = <DATA>;
  my $numValid = countPassports(\@data);
  is($numValid, 133);
};

#note Dumper \%fieldsIHaveInvalidated;
...
sub countPassports{
  ...
  for(@$data){
    if(/^\s*$/){
      my $isValid = isPassportValid($currPassport);
      if($isValid){
        $numValid++;
      }
      $currPassport = "";
  ...
  if(isPassportValid($currPassport)){
    $numValid++;
  }
...
sub isPassportValid{
  my($passport) = @_;
  # remove whitespace
  # but first change newlines to spaces
  $passport =~ s/\n+/ /gm;
  $passport =~ s/^\s+|\s+$//g;
  ...
  # Required fields in the passport
  # cid optional; not counting here.
  my %required = (
    byr => sub{
      my $v = shift;
      $v=~/^\d{4}$/ && $v>=1920 && $v<=2002
  ...
  # Split by whitespace but then for each item,
  ...
  # Does each @required exist in @keyValue?
  while(my($r, $validationSub) = each(%required)){
    if(!$passport{$r}){
      #note "NOT FOUND $r";
      #note $passport;
      return 0;
    }
    my $passportValue = $passport{$r};
    my $validationSub = $required{$r};
    my $fieldValidated = $validationSub->($passportValue);
    if(!$fieldValidated){
      #note "Field not validated: $r => $passportValue";
      $fieldsIHaveInvalidated{$r}++;
      return 0;
    }
  }
  return 1;
