...
Monkey 3:
  ...
  for(my $round = 1; $round <= 10000; $round++){
    ...
    if($round == 1){
      is_deeply(\@inspections, [2,4,3,6], "inspections on round $round");
    }
    if($round == 20){
      is_deeply(\@inspections, [99,97,8,103], "inspections on round $round");
    }
    if($round == 1000){
      is_deeply(\@inspections, [5204,4792,199,5192], "inspections on round $round");
    }
    if($round == 10000){
      is_deeply(\@inspections, [52166,47830,1938,52013], "inspections on round $round");
    }
      
  }

  my @sortedInspections = sort {$b<=>$a} @inspections;
  is(product(@sortedInspections[0..1]), 2713310158, "product of inspections, monkey business");
};
...
subtest "Real $0" => sub{
  ...
  my @sortedInspections = sort {$b<=>$a} @inspections;
  is(product(@sortedInspections[0..1]), 19457438264, "product of inspections, monkey business");
...
sub finishARound{
  ...
  my $commonDenominator = product( map{$$_{modulo}} @$monkeys );
  ...
  for(my $i=0;$i<@$monkeys;$i++){
    ...
    for(my $j=0;$j<@$items;$j++){
      ...
      push(@{$$monkeys[$otherMonkeyIdx]{items}}, ($$items[$j] % $commonDenominator));
...
sub worryLevels{
  ...
  for my $item(@$items){
    ...
    # worry levels no longer are divided by 3
    #$worry = int($worry / 3);
    push(@worry, $worry);
