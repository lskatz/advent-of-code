...
readPositions();
while(1){
  my $was_printed = printPositions();
  advancePositions();
  if(!$was_printed){
    next;
  }
  print "<enter> to see the next positions ($numSeconds seconds)\n";
  my $response = <>;
  if($response=~/^q/i){
    last;
  }
...
sub readPositions{
  my $z;
  while(<DATA>){
    my @match = /position=<(.)?(\d+), (.)?(\d+)> velocity=<(.)?(\d+), (.)?(\d+)>/;
    my $x=$2;
      $x = -1 * $x if($1 eq '-');
    my $y=$4;
      $y = -1 * $y if($3 eq '-');
...
sub printPositions{
  ...
  # Print
  for(my $j=0;$j<=$maxY-$minY;$j++){
    for(my $i=0;$i<=$maxX-$minX;$i++){
      $checkerboard[$i][$j] //= '.';
      print $checkerboard[$i][$j];
    }
    print "\n";
  }
