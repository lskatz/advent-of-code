...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green
  ...
  my $valids = markValidGames($data);

  subtest "individual games validity" => sub{
    plan tests => 5;
    is($$valids{1}, 1);
    is($$valids{2}, 1);
    is($$valids{3}, 0);
  ...
  my @valid_ids;
  for my $k (keys(%$valids)){
    if($$valids{$k}){
      push(@valid_ids, $k);
    }
  }
  is(sum(@valid_ids), 8, "Sum of valid IDs");
};
...
subtest "Real $0" => sub{
  ...
  is(sum(@valid_ids), 2447, "Sum of valid IDs");
};
