...
.664.598..
  ...
  my $ints = findIntsNextToChars($data);

  my @exp = qw(467 35 633 617 592 755 664 598);

  is_deeply([sort @$ints], [sort @exp], "individual numbers");

  is(sum(@$ints), 4361, "sum of ints");
};
...
subtest "Real $0" => sub{
  ...
  # 532460 is too high
  is(sum(@$ints), 527369, "sum of ints");
};

sub findIntsNextToChars{
  my($data) = @_;
  ...
  # Find locations of special characters
  my $numLines = scalar(@data);
  my %charPos;
  for(my $i=0; $i < $numLines; $i++){
    for(my $j=0; $j< @{ $data[$i] }; $j++){
      if($data[$i][$j] !~ /[0-9\.]/){
        $charPos{$i}{$j} = $data[$i][$j];
      }
    }
  }
  #note Dumper $charPos{$numLines - 3};

  # Go through the original data again and find where numbers appear.
  # Compare those number positions to positions in %charPos.
  my @ints;
  for(my $i=0; $i<$numLines; $i++){
    my $line = join("", @{$data[$i]});
    #note "num lines: $numLines J:".length($line);
    while($line =~ /(\d+)/g){
      my $j = $-[0];
      my $num = $1;

      # Are there any special characters around this int?
      my $valid = 0;
      my $iUp   = max(0, $i-1);
      my $iDown = min($numLines-1, $i+1);
      my $jLeft = max(0, $j-1);
      my $jRight= min(length($line)-1, $j+length($num));
      #note "$num: I: $iUp - $iDown; J: $jLeft - $jRight";
