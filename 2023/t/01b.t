...
subtest "Test $0" => sub{
  ...
  my $calibration = calibrationData($data);
  #print Dumper $calibration;
  my @exp = (29, 83, 13, 24, 42, 14, 76);
  is_deeply($calibration, \@exp, "individual values");
  is(sum(@$calibration), 281, "sum");
};
...
subtest "Real $0" => sub{
  ...
  my $data = <DATA>;
  my $calibration = calibrationData($data);
  my $sum = sum(@$calibration);
  is($$calibration[0], 57, "first value");
  is($$calibration[1], 61, "second value");
  is($$calibration[2], 27, "third value");
  is($$calibration[3], 25, "fourth value");
  is($$calibration[4], 42, "fifth value");
  is($$calibration[5], 61, "sixth value on six7sixqrdfive3twonehsk");
  is($$calibration[6], 12, "seventh value");
  is($$calibration[7], 15, "eighth value");
  is($$calibration[-3], 84, "third to last value");
  is($$calibration[-2], 88, "second to last value");
  is($$calibration[-1], 31, "last value");
  is($sum, 53312, "sum");
};
...
sub calibrationData{
  ...
  for my $line(split(/\n/, $data)){
    ...
    for(my $i=0; $i<length($line); $i++){
      while(my($word,$int)=each(%wordToNum)){
        my $length = length($word);
        if(substr($line, $i, $length) eq $word){
          push(@ints, $int);
          next;
        }
      }
    }
    my $cal = ($ints[0].$ints[-1])+0;
    #note "$line => $cal";
    push(@calibration, $cal);
  }
