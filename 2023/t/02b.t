...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green
  ...
  my $needed = maxForEachColor($data);

  subtest 'min needed powers' => sub{
    plan tests => 5;

    is($$needed{1}{power}, 48);
    is($$needed{2}{power}, 12);
    is($$needed{3}{power}, 1560);
    is($$needed{4}{power}, 630);
    is($$needed{5}{power}, 36);
  };

  my $sum = sum( map{ $$needed{$_}{power} } keys(%$needed));
  is($sum, 2286, "sum of powers");
};
...
sub maxForEachColor{
  ...
  for my $line(split(/\n/, $data)){
    ...
    my($id, $draws) = split(/:\s*/, $line);
    $id =~ s/\D+//g;
    
    my @draw = split(/\s*;\s*/, $draws);
    for my $d(@draw){
      while($d =~ /(\d+) (\w+),?/g){
        my($count, $color) = ($1, $2);
        $maxColor{$color} //= 0;
    ...
    my $power = 1;
    for my $v(values(%maxColor)){
      $power *= $v;
    }
    $maxColor{power} = $power;

    $minNeeded{$id} = \%maxColor;
  }
