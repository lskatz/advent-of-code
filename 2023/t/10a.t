...
.....
  ...
  my $steps = stepsToFarthestPoint($data);
  is($steps, 4, "steps in square simple test");
...
LJ...
  ...
  my $steps2 = stepsToFarthestPoint($complexData);
  is($steps2, 8, "steps in square complex test");
};
...
subtest "Real $0" => sub{
  ...
  my $steps = stepsToFarthestPoint($data);
  is($steps, 6812, "steps in square simple test");
};

sub stepsToFarthestPoint{
  my($data) = @_;

  my $graph = readData($data);

  my @node = $graph->nodeList;
  my $startNode = (grep{$_->{label} eq 'S'} @node)[0];
  if(!defined $startNode){
    die "ERROR: could not find start node";
  }

  my %solution = (originID => $$startNode{id});
  my $dist = $graph->farthestNode(\%solution);
  #note Dumper \%solution, $dist;
...
sub readData{
  ...
  my @data = grep {/./} map{s/^\s+|\s+$//g;$_;} split(/\n/, $data);

  # Get all the characters into a 2d array
  for(my $i=0;$i<@data;$i++){
    my @chr = split(//, $data[$i]);
    $plot[$i] = \@chr;
  }

  # What points in eaach direction?
  # Compile regular expressions to save time later.
  my $connects_south = qr/^[\|7FS]$/;
  my $connects_north = qr/^[\|LJS]$/;
  my $connects_west  = qr/^[\-J7S]$/;
  my $connects_east  = qr/^[\-LFS]$/;

  # Get a new 2d array with pointers to the correct cells
  ...
  for(my $i=0;$i<@plot;$i++){
    for(my $j=0;$j<@{$plot[$i]};$j++){
      ...
      # Need to match these characters later
      my $sourceChar = $plot[$i][$j] or die "source char not defined at plot $i,$j";
      my $north = ($i-1 >= 0) ? $plot[$i-1][$j] : '.';
      my $west  = ($j-1 >= 0) ? $plot[$i][$j-1] : '.';
      my $east  = $plot[$i][$j+1] // '.';
      my $south = $plot[$i+1][$j] // '.';

      # Dots don't connect to anything and so we aren't making an edge
      if($plot[$i][$j] eq '.'){
        next;
      }

      # Make nodes
      for my $id("$i,$j", ($i+1).",$j", ($i-1).",$j", "$i,".($j+1), "$i,".($j-1)){
        $graph->node({id=>$id});
      }
      if($sourceChar eq 'S'){
        $graph->node({id=>"$i,$j", label=>"S"});
      }

      if($sourceChar =~ $connects_south and $south =~ $connects_north){
        #note "N->S edge for $sourceChar/$south at $i,$j";
        $graph->edge({sourceID=>$sourceID, targetID=>($i+1).",$j", weight=>1});
      }
      if($sourceChar =~ $connects_north and $north =~ $connects_south){
        $graph->edge({sourceID=>$sourceID, targetID=>($i-1).",$j", weight=>1});
      }
      if($sourceChar =~ $connects_east  and $east  =~ $connects_west ){
        $graph->edge({sourceID=>$sourceID, targetID=>"$i,".($j+1), weight=>1});
      }
      if($sourceChar =~ $connects_west  and $west  =~ $connects_east ){
        $graph->edge({sourceID=>$sourceID, targetID=>"$i,".($j-1), weight=>1});
      }
