...
.664.598..
  ...
  my $gears = findValidGears($data);

  subtest 'individual gears' => sub{
    plan tests => 2;

    is_deeply($$gears[0], {0=>467, 2=>35});
    is_deeply($$gears[1], {7=>755, 9=>598});
  };

  my @product;
  for(my $i=0;$i<@$gears;$i++){
    $product[$i] = 1;
    for my $v(values(%{ $$gears[$i] })){
      $product[$i] *= $v;
    }
  }
  is(sum(@product), 467835, "sum of products");
...
subtest "Real $0" => sub{
  ...
  # 532460 is too high
  is(sum(@product), 73074886, "sum of products");
};

sub findValidGears{
  my($data) = @_;

  # Push all the data into an array of arrays
  my @data;
  for my $line(split(/\n/, $data)){
    next if($line =~ /^\s*$/);
    chomp $line;
  ...
  # coordinates => uniqueID. uniqueID is the array element in @ints.
  my %ints;
  # list of integers in the matrix: array id (uniqueID) => actual integer
  my @ints;
  for(my $i=0; $i < $numLines; $i++){
    ...
    while($line =~ /(\d+)/g){
      ...
      push(@ints, $theInt);
      my $j = $-[0];
      my $jRight = $j+length($theInt)-1;
      for($j..$jRight){
        $ints{$i}{$_} = (scalar(@ints)-1);
      }
    }
  }

  # find valid asterisk characters
  my @validGears;
  for(my $i=0; $i < $numLines; $i++){
    my $line = join("", @{$data[$i]});
    while($line =~ /(\*)/g){
      ...
      # uniqueID => actual integer
      my %whichIntegers;
      my $iUp   = max(0, $i-1);
      my $iDown = min($numLines-1, $i+1);
      my $jLeft = max(0, $j-1);
      my $jRight= min(length($line)-1, $j+1);
      #note "I: $iUp - $iDown; J: $jLeft - $jRight";

      for(my $I=$iUp; $I<=$iDown; $I++){
        for(my $J=$jLeft; $J<=$jRight; $J++){
          if(defined $ints{$I}{$J}){
            $whichIntegers{ $ints{$I}{$J} } = $ints[ $ints{$I}{$J} ];
          }
        }
      }

      # It is only a gear if exactly two integers are next to it
      if(scalar(keys(%whichIntegers)) == 2){
        push(@validGears, \%whichIntegers);
      }

      #note Dumper \%ints, \%whichIntegers;
    }
  }

  #note Dumper \@validGears;die;
  return \@validGears;
